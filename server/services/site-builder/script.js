...
function renderVersionMenu(versions) {
  ...
  for (var i = 0; i < versions.length; i++) {
    ...
    item.innerHTML = '<span class="vp-vs-item-label">' + escH(label) + badge + (isCurrent ? ' <span class="vp-vs-item-current-tag">当前</span>' : '') + '</span>' +
      (v.change_notes ? '<span class="vp-vs-item-notes">' + escH(v.change_notes.slice(0, 40)) + (v.change_notes.length > 40 ? '...' : '') + '</span>' : '');
