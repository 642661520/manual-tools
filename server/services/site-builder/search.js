...
function renderResults() {
  ...
  for (var i = 0; i < _searchResults.length; i++) {
    ...
    if (e.section) titleHtml += ' <span class="vp-search-result-sep">&middot;</span> ' + highlightTitle(e.section, terms);
    var activeClass = i === _searchSelected ? ' active' : '';

    html += '<div class="vp-search-result-item' + activeClass + '" data-index="' + i + '" onclick="navigateResult(' + i + ')">' +
      '<div class="vp-search-result-title">' + titleHtml + '</div>' +
      '<div class="vp-search-result-snippet">' + snippet + '</div>' +
      '</div>';
  }
  el.innerHTML = html;
