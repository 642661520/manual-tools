...
export async function diffRoutes(app: FastifyInstance) {
  app.get('/api/v1/catalogs/:id/diff', { preHandler: authMiddleware }, async (req, reply) => {
    ...
    const db = getDb()
    const catalog = db.prepare('SELECT project_id FROM catalogs WHERE id = ?').get(id) as { project_id: string } | undefined
    ...
    const v1Row = db.prepare('SELECT * FROM catalog_versions WHERE catalog_id = ? AND id = ?').get(id, v1) as { version_major: number; version_minor: number; title: string; markdown: string } | undefined
    const v2Row = db.prepare('SELECT * FROM catalog_versions WHERE catalog_id = ? AND id = ?').get(id, v2) as { version_major: number; version_minor: number; title: string; markdown: string } | undefined
    ...
    for (const c of changes) {
      for (const line of c.value.split('\n')) {
        ...
        if (c.added) { lines.push({ type: 'added', text: line }); added++ }
        else if (c.removed) { lines.push({ type: 'removed', text: line }); removed++ }
        else { lines.push({ type: 'unchanged', text: line }); unchanged++ }
      }
    }
