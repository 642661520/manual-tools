...
export async function projectRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/projects', { preHandler: [authMiddleware, requireRole('admin')] }, async (req, reply) => {
    ...
    // 创建者自动加入项目成员为 pm
    db.prepare('INSERT OR IGNORE INTO project_members (project_id, user_id, role) VALUES (?, ?, ?)').run(
      id, req.user!.userId, 'pm',
    )
  ...
  // 更新项目（admin only）
  app.put('/api/v1/projects/:id', { preHandler: [authMiddleware, requireRole('admin')] }, async (req, reply) => {
    const { id } = req.params as { id: string }
  ...
  app.delete('/api/v1/projects/:id', { preHandler: [authMiddleware, requireRole('admin')] }, async (req, reply) => {
    ...
    const catalogIds = (db.prepare('SELECT id FROM catalogs WHERE project_id = ?').all(id) as Array<{ id: string }>).map(r => r.id)
    ...
    for (const cid of catalogIds) {
      ...
      rm(docsDir, { recursive: true, force: true }).catch(err => {
        app.log.error(`清理文档站点失败 (catalog ${cid}): ${err instanceof Error ? err.message : err}`)
      })
    }
  ...
  app.post('/api/v1/projects/:id/members', { preHandler: [authMiddleware, requireRole('admin', 'member')] }, async (req, reply) => {
    const { id } = req.params as { id: string }
    const userId = req.user!.userId
    const role = req.user!.role
    ...
    const { userId: targetUserId, projectRole } = req.body as { userId: string; projectRole?: string }
    if (!targetUserId) {
    ...
    // 检查是否已是项目成员
    const existingMember = db.prepare('SELECT role FROM project_members WHERE project_id = ? AND user_id = ?').get(id, targetUserId) as { role: string } | undefined
    const isExistingMember = !!existingMember

    db.prepare('INSERT OR IGNORE INTO project_members (project_id, user_id, role) VALUES (?, ?, ?)').run(id, targetUserId, memberRole)
    // 如果已存在，更新角色
    db.prepare('UPDATE project_members SET role = ? WHERE project_id = ? AND user_id = ?').run(memberRole, id, targetUserId)
    ...
    if (isExistingMember) {
      ...
      if (existingMember.role !== memberRole) {
        notifyProjectRoleChange(projectName, targetUserId, existingMember.role, memberRole, operatorName)
          .catch(e => app.log.error(`飞书通知失败(项目角色变更) ${e instanceof Error ? e.message : e}`))
      }
    } else {
      // 新加入通知
      notifyJoinProject(projectName, targetUserId, operatorName, memberRole)
        .catch(e => app.log.error(`飞书通知失败(加入项目) ${e instanceof Error ? e.message : e}`))
    }
  ...
  app.delete('/api/v1/projects/:id/members/:userId', { preHandler: [authMiddleware, requireRole('admin', 'member')] }, async (req, reply) => {
    ...
    // 通知被移出的用户
    notifyLeaveProject(project?.name || id, targetUserId, operator?.display_name || '系统管理员')
  ...
  app.put('/api/v1/projects/:id/review-chain', { preHandler: [authMiddleware, requireRole('admin', 'member')] }, async (req, reply) => {
    const { id } = req.params as { id: string }
    const userId = req.user!.userId
    const role = req.user!.role
