...
export async function catalogRoutes(app: FastifyInstance) {
  ...
  // 预览单章
  app.get(
    '/api/v1/catalogs/:id/chapters/:chNum',
  ...
  app.get(
    '/api/v1/catalogs/:id/versions/:versionId/preview',
    { preHandler: authMiddleware },
    async (req, reply) => {
      const { versionId } = req.params as { versionId: string }
      const { mode } = req.query as { mode?: string }
      const db = getDb()
      const ver = db.prepare('SELECT * FROM catalog_versions WHERE id = ?').get(versionId) as
        | CatalogVersionRow
        | undefined
  ...
  // 历史版本单章预览
  app.get(
    '/api/v1/catalogs/:id/versions/:versionId/chapters/:chNum',
    { preHandler: authMiddleware },
    async (req, reply) => {
      const {
        id,
        versionId,
        chNum: chNumStr,
      } = req.params as { id: string; versionId: string; chNum: string }
      const { mode } = req.query as { mode?: string }
      const chNum = parseInt(chNumStr)
      if (isNaN(chNum) || chNum < 1) return fail(reply, 400, '无效的章节编号')
      const db = getDb()
      ...
      const chId = `ch${chNum}`
      const allHeadings = JSON.parse(ver.headings_json || '[]') as Array<{
        level: number
        text: string
        id: string
      }>
  ...
  app.get(
    '/api/v1/catalogs/:id/export/markdown',
    ...
    async (req, reply) => {
      ...
      if (cached) {
        ...
        reply.header(
          'Content-Disposition',
          `attachment; filename="${encodeURIComponent(cached.fileName)}"`,
        )
  ...
  // 历史版本导出 PDF
  app.get(
    '/api/v1/catalogs/:id/versions/:versionId/export/pdf',
    { preHandler: [authMiddleware, requireRole('admin', 'member')] },
    async (req, reply) => {
      ...
      const approvedOnly = mode === 'approved'
      const statusOv = approvedOnly
        ? (JSON.parse(ver.status_snapshot || '{}') as Record<string, string>)
        : undefined
      const featuresData = JSON.parse(ver.features_json || '[]') as FeatureData[]
      const manual = assembleManual(ver.catalog_id, {
        approvedOnly,
      ...
      try {
        ...
        const safeTitle = (ver.title || catalogMeta.title)
          .replace(/[\\/:*?"<>|]/g, '_')
          .slice(0, 50)
  ...
  // 发布版本（项目 pm+ 可操作）
  app.post(
    '/api/v1/catalogs/:id/publish',
    { preHandler: [authMiddleware, requireRole('admin', 'member')] },
    async (req, reply) => {
      const { id } = req.params as { id: string }
      const body = req.body as { changeNotes?: string; visibility?: string; approvedOnly?: boolean }
      const visibility = (
        ['public', 'login_required', 'project_members'].includes(body.visibility || '')
          ? body.visibility
          : 'project_members'
      ) as string
      ...
      for (const fe of featuresList) {
        const f = db
          .prepare('SELECT title, sections FROM features WHERE id = ?')
  ...
  // 更新版本可见性
  app.put(
    '/api/v1/catalogs/:id/versions/:versionId/visibility',
    { preHandler: [authMiddleware, requireRole('admin', 'member')] },
    async (req, reply) => {
      const { id, versionId } = req.params as { id: string; versionId: string }
      ...
      const db = getDb()
      const ver = db
        .prepare('SELECT catalog_id FROM catalog_versions WHERE id = ?')
        .get(versionId) as { catalog_id: string } | undefined
      if (!ver || ver.catalog_id !== id) return fail(reply, 404, '版本不存在')
  ...
  app.get('/api/v1/catalogs/:id', { preHandler: authMiddleware }, async (req, reply) => {
    ...
    function resolveFeature(fe: CatalogFeatureEntry) {
      ...
      if (!f) return null
      const sections = JSON.parse(f.sections || '[]') as {
        key: string
        title: string
        description?: string
      }[]
  ...
  app.put(
    ...
    async (req, reply) => {
      ...
      const existing = db.prepare('SELECT id, project_id FROM catalogs WHERE id = ?').get(id) as
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      const catalog = db.prepare('SELECT id, project_id FROM catalogs WHERE id = ?').get(id) as
        | { id: string; project_id: string }
        | undefined
      if (!catalog) return fail(reply, 404, 'Not found')
      ...
      rm(docsDir, { recursive: true, force: true }).catch((err) => {
        app.log.error(
          `清理文档站点失败 (catalog ${id}): ${err instanceof Error ? err.message : err}`,
        )
      })
