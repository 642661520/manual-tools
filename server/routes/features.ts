...
function getEffectiveReviewChain(projectId: string): string[] {
  const db = getDb()
  const project = db.prepare('SELECT review_chain FROM projects WHERE id = ?').get(projectId) as
    | { review_chain: string }
    | undefined
  if (!project) return []
...
export async function featureRoutes(app: FastifyInstance) {
  ...
  app.put(
    '/api/v1/features/:id/sections/:sectionKey/status',
    { preHandler: [authMiddleware, requireRole('admin', 'member')] },
    async (req, reply) => {
      ...
      const existingDoc = db.prepare('SELECT * FROM documents WHERE id = ?').get(docId) as
        | DocumentRow
        | undefined
      ...
      const result = db.transaction(() => {
        // 确保 document 存在
        const doc = db
          .prepare('SELECT id, status_log, assignees FROM documents WHERE id = ?')
          .get(docId) as { id: string; status_log: string; assignees: string } | undefined
        if (!doc) {
          db.prepare(
        ...
        const statusLog = JSON.parse(doc?.status_log || '[]') as {
          action: string
          userId: string
          note: string
          step: number
        ...
        if (assignees !== undefined) {
          ...
          if (added.length > 0) {
            jobs.push(() =>
              notifyAssignees(featureId, sectionKey, added, operatorName).catch((e: unknown) =>
                app.log.error(`飞书通知失败(指派) ${e instanceof Error ? e.message : e}`),
              ),
            )
          }
          ...
          for (const uid of removed) {
            jobs.push(() =>
              notifyRemoveAssignee(featureId, sectionKey, uid, operatorName).catch((e: unknown) =>
                app.log.error(`飞书通知失败(移除指派) ${e instanceof Error ? e.message : e}`),
              ),
            )
          }
        }
        ...
        if (reviewNote !== undefined && !status) {
          db.prepare(
            "UPDATE documents SET review_note = ?, updated_at = datetime('now') WHERE id = ?",
          ).run(reviewNote || '', docId)
          return ok()
        }
        ...
        if (status === 'pending_review') {
          addLog('submitted', '', 0)
          db.prepare(
            "UPDATE documents SET status = 'pending_review', review_step = 0, status_log = ?, updated_at = datetime('now') WHERE id = ?",
          ...
          jobs.push(() =>
            notifyNextReviewer(
              ...
              app.log.error(`飞书通知失败(提交) ${e instanceof Error ? e.message : e}`),
            ),
          )
          return ok()
        }
        ...
        if (status === 'approved') {
          if (direct) {
            ...
            db.prepare(
              "UPDATE documents SET status = 'approved', status_log = ?, review_note = ?, updated_at = datetime('now') WHERE id = ?",
            ).run(JSON.stringify(statusLog), '', docId)
            if (oldAssignees.length > 0) {
              jobs.push(() =>
                notifyDirectApprove(
                  ...
                  app.log.error(`飞书通知失败(直接通过) ${e instanceof Error ? e.message : e}`),
                ),
              )
            }
          } else {
            ...
            if (newStep >= reviewChain.length) {
              db.prepare(
                "UPDATE documents SET status = 'approved', review_step = ?, status_log = ?, review_note = ?, updated_at = datetime('now') WHERE id = ?",
              ...
              if (oldAssignees.length > 0) {
                jobs.push(() =>
                  notifyWriterReviewResult(
                    ...
                    app.log.error(`飞书通知失败(通过) ${e instanceof Error ? e.message : e}`),
                  ),
                )
              }
            } else {
              db.prepare(
                "UPDATE documents SET review_step = ?, status_log = ?, review_note = ?, updated_at = datetime('now') WHERE id = ?",
              ).run(newStep, JSON.stringify(statusLog), reviewNote || '', docId)
              jobs.push(() =>
                notifyNextReviewer(
                  featureId,
                  ...
                  app.log.error(`飞书通知失败(推进) ${e instanceof Error ? e.message : e}`),
                ),
              )
            }
          }
        ...
        if (status === 'rejected') {
          ...
          db.prepare(
            "UPDATE documents SET status = 'rejected', status_log = ?, review_note = ?, updated_at = datetime('now') WHERE id = ?",
          ...
          if (oldAssignees.length > 0) {
            jobs.push(() =>
              notifyWriterReviewResult(
                ...
                app.log.error(`飞书通知失败(退回) ${e instanceof Error ? e.message : e}`),
              ),
            )
          }
          return ok()
        ...
        if (status === 'draft' || status === 'in_progress') {
          ...
          db.prepare(
            "UPDATE documents SET status = ?, review_step = 0, status_log = ?, updated_at = datetime('now') WHERE id = ?",
          ...
          if (oldAssignees.length > 0) {
            jobs.push(() =>
              notifyStatusReset(
                ...
                app.log.error(`飞书通知失败(重置) ${e instanceof Error ? e.message : e}`),
              ),
            )
          }
          return ok()
  ...
  // 删除游离文档（pm only）
  app.delete(
    '/api/v1/features/:id/orphaned/:sectionKey',
    { preHandler: [authMiddleware, requireRole('admin', 'member')] },
    async (req, reply) => {
      const { id: featureId, sectionKey } = req.params as { id: string; sectionKey: string }
      const db = getDb()
  ...
  app.put(
    ...
    async (req, reply) => {
      ...
      const db = getDb()
      const feature = db.prepare('SELECT id, project_id FROM features WHERE id = ?').get(id) as
        | Pick<FeatureRow, 'id' | 'project_id'>
        | undefined
      if (!feature) return fail(reply, 404, 'Not found')
