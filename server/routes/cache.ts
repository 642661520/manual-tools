...
export async function cacheRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const db = getDb()
      const entries = db
        .prepare('SELECT id, file_path FROM export_cache WHERE catalog_id = ?')
  ...
  // 下载导出缓存文件
  app.get(
    '/api/v1/cache/entries/export/:id/download',
    { preHandler: [authMiddleware, requireRole('admin', 'pm')] },
    async (req, reply) => {
      ...
      const entry = db
        .prepare('SELECT file_path, file_name, type FROM export_cache WHERE id = ?')
      ...
      const mimeType = entry.type === 'pdf' ? 'application/pdf' : 'application/zip'

      reply.header('Content-Type', mimeType)
      reply.header(
        'Content-Disposition',
        `attachment; filename="${encodeURIComponent(entry.file_name)}"`,
      )
  ...
  app.get(
    ...
    async (req) => {
      ...
      const db = getDb()
      const total = (
        db.prepare('SELECT COUNT(*) as cnt FROM remote_cache').get() as { cnt: number }
      ).cnt
      const rows = db
        .prepare(`
  ...
  // 预览远程缓存文件（图片/视频等）
  app.get(
    '/api/v1/cache/entries/remote/:hash/:ext/preview',
    { preHandler: [authMiddleware, requireRole('admin', 'pm')] },
    async (req, reply) => {
      ...
      const db = getDb()
      const row = db
        .prepare('SELECT ext, mime_type FROM remote_cache WHERE hash = ? LIMIT 1')
  ...
  // 删除单个导出缓存文件
  app.delete(
    '/api/v1/cache/entries/export/:id',
    { preHandler: [authMiddleware, requireRole('admin', 'pm')] },
    async (req, reply) => {
      const { id } = req.params as { id: string }
      const db = getDb()
  ...
  // 删除单个远程缓存条目
  app.delete(
    '/api/v1/cache/entries/remote/:hash/:ext',
    { preHandler: [authMiddleware, requireRole('admin', 'pm')] },
    async (req, reply) => {
      const { hash, ext } = req.params as { hash: string; ext: string }
      ...
      // 统计同一 hash 被多少条记录引用
      const refCount = (
        db.prepare('SELECT COUNT(*) as cnt FROM remote_cache WHERE hash = ?').get(hash) as {
          cnt: number
        }
      ).cnt
