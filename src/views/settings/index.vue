...
type Tab = 'projects' | 'users' | 'backup' | 'storage'
const validTabs: Tab[] = ['projects', 'users', 'backup', 'storage']
        ...
        <div class="card">
          ...
          <div v-else-if="cacheStats" class="text-xs text-gray-500 space-y-1 mb-3">
            ...
            <div class="flex items-center gap-2">
              ...
              <button v-if="cacheStats.export.count > 0" class="text-blue-400 hover:text-blue-600 ml-1" :disabled="cacheEntriesLoading" @click="loadExportEntries">
            ...
            <table v-else class="w-full text-xs">
              <thead>
                <tr class="text-left text-gray-400 border-b border-gray-100">
                  ...
                  <th class="py-1.5 font-medium w-6"></th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="entry in exportEntries" :key="entry.id" class="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td class="py-1.5 pr-2 truncate max-w-[200px]" :title="entry.fileName">{{ entry.fileName }}</td>
                  <td class="py-1.5 pr-2 truncate max-w-[160px] text-gray-500" :title="entry.catalogTitle">{{ entry.catalogTitle }}</td>
                  ...
                  <td class="py-1.5">
                    <button class="text-red-400 hover:text-red-600" title="删除此缓存" @click="handleDeleteExportEntry(entry)">
              ...
              <div class="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-2">
                ...
                >
                  ...
                  <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent px-1.5 py-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div class="text-[10px] text-white truncate" :title="entry.url">{{ shortUrl(entry.url, 25) }}</div>
    ...
    >
      <div class="relative bg-white rounded-xl shadow-2xl max-w-4xl max-h-[90vh] flex flex-col overflow-hidden" @click.stop>
        ...
        <div class="px-4 py-2.5 border-t border-gray-100 text-xs text-gray-400 space-y-0.5 bg-white">
          <div class="truncate" :title="previewEntry.url">
