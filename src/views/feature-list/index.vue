...
const featureForm = ref({ title: '', description: '', categoryId: null as string | null, sections: [] as SectionDef[] })
...
async function saveFeature() {
  ...
  try {
    ...
    featureForm.value = { title: '', description: '', categoryId: null, sections: [] }
    await loadFeatures()
  } catch (e: unknown) { featureFormError.value = e instanceof Error ? e.message : '网络错误，保存失败' }
}

function initFeatureSectionSort() {
  if (featureSectionSortable) { featureSectionSortable.destroy(); featureSectionSortable = null }
    ...
    <div v-if="showFeatureDialog" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col">
            ...
            <div>
              ...
              <div v-if="featureForm.sections.length > 0" ref="featureSectionSortEl" class="mb-3 space-y-1">
                <div v-for="(s, i) in featureForm.sections" :key="s.key" class="flex items-center gap-2 text-sm bg-gray-50 px-3 py-1.5 rounded">
                  ...
                  <button class="text-red-400 hover:text-red-600 text-xs p-1" @click="removeSection(i)"><span class="i-lucide-x w-4 h-4 inline-block align-middle" /></button>
                </div>
              </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3 flex-shrink-0">
          ...
          <button class="btn-primary" @click="saveFeature">{{ featureDialogMode === 'create' ? '创建' : '保存' }}</button>
        </div>
      </div>
    </div>

    <!-- 分类管理 -->
    <div v-if="showCategoryDialog" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h2 class="text-lg font-semibold">分类管理</h2>
          <button class="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100" @click="showCategoryDialog = false"><span class="i-lucide-x w-4 h-4 inline-block align-middle" /></button>
        </div>
        <div class="p-6">
          ...
          <div class="flex items-end gap-2 pb-4 border-b border-gray-100">
            <FormField label="分类名称" class="flex-1">
              <input v-model="newCategory.name" class="input" placeholder="如：系统管理" @keyup.enter="createCategory" />
            </FormField>
            <FormField label="颜色">
          ...
          <div v-if="categories.length > 0" ref="categorySortEl" class="mt-4 space-y-1">
            <div v-for="c in categories" :key="c.id"
              ...
              <div class="cat-drag-handle cursor-grab text-gray-300 hover:text-gray-500 flex-shrink-0">
          ...
          <p v-if="categories.length > 1" class="text-xs text-gray-400 mt-3">拖拽 <span class="i-lucide-grip-vertical w-3 h-3 inline-block align-middle" /> 图标可调整分类排序</p>
        </div>
