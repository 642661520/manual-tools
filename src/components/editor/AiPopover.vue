...
<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-[60]" @click.self="() => emit('close')">
      <div
        class="absolute left-1/2 top-[20vh] -translate-x-1/2 bg-surface rounded-xl shadow-2xl border border-default w-[480px] max-w-[95vw] overflow-hidden"
      >
        ...
        <div class="px-4 py-3 space-y-3">
          <div class="flex items-start gap-2 text-xs bg-base rounded-lg p-2.5 border border-light">
            <span
              class="i-lucide-mouse-pointer-click w-3.5 h-3.5 text-muted flex-shrink-0 mt-0.5"
            />
            <span class="text-secondary line-clamp-2">{{ selectedText }}</span>
          </div>
          ...
          <div class="flex gap-2">
            ...
            >
              <span class="text-sm font-medium text-secondary">{{ p.label }}</span>
              <span class="text-[10px] text-muted">{{ p.desc }}</span>
            </button>
          </div>
          ...
          <div
            v-if="loading"
            class="flex items-center gap-2 py-4 justify-center text-sm text-muted"
          >
            <span class="i-lucide-loader-2 w-4 h-4 animate-spin" />
            正在处理「{{ selectedText.slice(0, 30) }}{{ selectedText.length > 30 ? '…' : '' }}」...
          </div>

          <div v-if="error" class="text-sm color-danger bg-danger rounded-lg p-3">
            {{ error }}
          </div>
