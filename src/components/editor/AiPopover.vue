...
const lastAction = ref<{ type: 'preset'; key: string } | { type: 'custom'; prompt: string } | null>(null)

watch(() => props.visible, (v) => {
  if (!v) { result.value = ''; error.value = ''; customPrompt.value = ''; lastAction.value = null }
})
...
async function runPreset(key: string) {
  ...
  } catch (e: unknown) { error.value = e instanceof Error ? e.message : '请求失败' }
...
async function retry() {
  ...
  else { customPrompt.value = lastAction.value.prompt; await runCustom() }
}

function apply() { emit('apply', result.value); result.value = '' }

function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape') { emit('close'); return }
  if (e.key === 'Enter' && customPrompt.value.trim()) { e.preventDefault(); runCustom() }
}
</script>
...
<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-[60]" @click.self="emit('close')">
      <div class="absolute left-1/2 top-[20vh] -translate-x-1/2 bg-white rounded-xl shadow-2xl border border-gray-200 w-[480px] max-w-[95vw] overflow-hidden">
        ...
        <div class="px-4 py-3 space-y-3">
          <div class="flex items-start gap-2 text-xs bg-gray-50 rounded-lg p-2.5 border border-gray-100">
            <span class="i-lucide-mouse-pointer-click w-3.5 h-3.5 text-gray-400 flex-shrink-0 mt-0.5" />
          ...
          <div class="flex gap-2">
            <button v-for="p in presets" :key="p.key"
              ...
              :class="{ 'opacity-50 pointer-events-none': loading }" :disabled="loading" @click="runPreset(p.key)">
          ...
          <div class="flex gap-2">
            <input v-model="customPrompt" type="text" class="input text-sm flex-1"
              placeholder="或输入自定义指令，如「翻译为英文」" :disabled="loading" @keydown="onKeydown" />
            <button class="btn-primary text-xs px-3 flex-shrink-0" :disabled="loading || !customPrompt.trim()" @click="runCustom">发送</button>
          </div>

          <div v-if="loading" class="flex items-center gap-2 py-4 justify-center text-sm text-gray-400">
          ...
          <div v-if="result && !loading" class="space-y-3">
            <div class="bg-violet-50 rounded-lg p-3 max-h-64 overflow-y-auto text-sm text-gray-700 leading-relaxed" v-html="result" />
            <div class="flex gap-2">
              <button class="btn-primary text-sm flex-1" @click="apply"><span class="i-lucide-check w-4 h-4 inline-block align-middle mr-1" />替换原文</button>
              <button class="btn-secondary text-sm" @click="retry"><span class="i-lucide-rotate-ccw w-4 h-4 inline-block align-middle mr-1" />重试</button>
            </div>
          </div>
        </div>
      </div>
